...
if (isset ($_GET['id']))
{
    ...
    if($idClient!=false)
    {
        $client= ClientsManager::findById($idProduit);
    }
}
...
switch($mode)
{
    case 'ajout':
    {
        echo'<form method="POST" action="index.php?code=formClient&mode=ajout">';
        break;        
    }
    case 'update':
    {
        echo'<form method="POST" action="index.php?code=formClient&mode=update"/>
        <input name="idProduit" value="'.$produit->getIdClient().'" type="hidden"/>';
        break;
    }
    case 'delete':
    {
        echo'<form method="POST" action="index.php?code=formClient&mode=delete">
        <input name="idProduit" value="'.$produit->getIdClient().'" type="hidden"/>';
        break;
    }
}
