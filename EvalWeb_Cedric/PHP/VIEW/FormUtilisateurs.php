...
<div>
        ...
        <input class="libelle"  name="login" <?php if($m != "ajouter") echo 'value= "'.$utilisateur->getLogin().'"';if($m=="supprimer") echo '" disabled'; ?>/>
        <div class="espace"></div>
    </div>
    <div>
        <div class="centre size">Nom:</div>
        <input class="libelle"  name="nom" <?php if($m != "ajouter") echo 'value= "'.$utilisateur->getNomUtilisateur().'"';if($m=="supprimer") echo '" disabled'; ?>/>
        <div class="espace"></div>
    </div> 
    <div class="colonne">
        <div>
        <div class="centre size">Prénom:</div>
        <input class="libelle"  name="prenom" <?php if($m != "ajouter") echo 'value= "'.$utilisateur->getPrenomUtilisateur().'"';if($m=="supprimer") echo '" disabled'; ?>/>
        <div class="espace"></div>
    </div>
    <div class="colonne">
        <div>
            <?php
            $id= UtilisateursManager::findById($id);
            ...
            foreach($liste as $uneMatiere)
            {
                if($uneMatiere->getIdMatiere()==$id->getIdMatiere())
                {
                    ...
                    <input class="libelle"  name="matiere"' ?><?php if($m != "ajouter") echo' value= "'.$uneMatiere->getLibelle().'"';if($m=="supprimer") echo '" disabled"/>
                    <div class="espace"></div>';
                }
            }
        ?>
